...
open class LineChartRenderer: LineRadarRenderer
{
    ...
    private func drawAccessibilityElementGeometry(dataSet: ILineChartDataSet, boundsIndex: Int, scale: CGFloat, pt: CGPoint) {
        guard
            let dataProvider = dataProvider,
            let lineData = dataProvider.lineData,
            let e = dataSet.entryForIndex(boundsIndex)
            else { return }
        let phaseY = animator.phaseY
        let trans = dataProvider.getTransformer(forAxis: dataSet.axisDependency)
        ...
        if !isGroupSelectionEnabled {
            ...
            if let chart = dataProvider as? LineChartView
            {
                let element = createAccessibleElement(withIndex: boundsIndex,
                                                        container: chart,
                                                        dataSet: dataSet,
            ...
            for set in dataSets {
                if(set.containsEntriesAtXValue(fromX: e.x, toX: e.x)) {
                    dataSetsWithDataBetween.append(set as! ILineChartDataSet)
                }
            }
            let dataSetYValues = dataSetsWithDataBetween
                .flatMap { $0.entriesForXValue(e.x) }
                .compactMap { $0.y }
            let minY = CGFloat(phaseY * dataSetYValues.min()!)
            let maxY = CGFloat(phaseY * dataSetYValues.max()!)
            var yMinPt = CGPoint()
            var yMaxPt = CGPoint()

            yMinPt.y = minY
            yMaxPt.y = maxY
            yMinPt = yMinPt.applying(valueToPixelMatrix)
            yMaxPt = yMaxPt.applying(valueToPixelMatrix)
            let accessibilityRect = CGRect(x: pt.x - (scaleFactor * circleRadius),
                                           y: yMaxPt.y - (scaleFactor * circleRadius),
                                           width: scaleFactor * circleDiameter,
                                           height: yMinPt.y - yMaxPt.y + (scaleFactor * circleRadius * 2))
            // Create and append the corresponding accessibility element to accessibilityOrderedElements
            if let chart = dataProvider as? LineChartView {
                let element = createAccessibleElement(container: chart, entry: e)
                { (element) in
                    element.accessibilityFrame = accessibilityRect
                }
    ...
    private func drawEntries(context: CGContext, dataSet: ILineChartDataSet, scale: CGFloat, alpha: CGFloat) {
        ...
        for j in _xBounds
        {
            ...
            drawAccessibilityElementGeometry(dataSet: dataSet, boundsIndex: j, scale: scale, pt: pt)
    ...
    private func createAccessibleElement(container: LineChartView,
